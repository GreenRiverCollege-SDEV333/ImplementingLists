...
public class SinglyLinkedList<ItemType> implements List<ItemType> {
    ...
    public void remove(ItemType item) {
        ...
        // alernative - easier to write but less efficient
       /* int position = indexOf(item);
        if (position != -1){
            remove(position);
        }*/
    ...
    public void add(int index, ItemType item) {
        ...
        if (index == 0) {
            // if someone wants to add in the beginning, I need to change the head
            ...
            // when i get here, current is pointing to the node BEFORE at the index
            Node theNewOne = new Node();
            theNewOne.data = item;
    ...
    public int indexOf(ItemType item) {
        ...
        // if we get here, it is not found
        return -1;
    }
